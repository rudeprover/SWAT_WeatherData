...
def build_human_metadata(ds, var_name):
    """Generate readable metadata summary for NetCDF files."""
    lines = []
...
def main():
    ...
    st.caption("Convert gridded climate data for SWAT modeling or extract NetCDF variables as CSV + Metadata.")
    ...
    if mode == "SWAT Builder":
        ...
        shapefile_upload = st.sidebar.file_uploader("Upload Boundary (Shapefile ZIP or GeoJSON)",
                                                    type=["zip", "geojson"])
        st.sidebar.header("📅 Date Range")
        ...
        st.info("Click below to begin extracting rainfall and temperature data for your chosen area and time period.")
        st.button("🚀 Generate SWAT-Compatible Weather Data", type="primary", use_container_width=True)

        # ----------------- About -----------------
        st.header("ℹ️ About — SWAT Builder")
        st.markdown("""
        This module processes **gridded rainfall and temperature datasets** into
        **SWAT-compatible station data**.

        **How it works:**
        - Upload your **shapefile or GeoJSON boundary** for the study area.
        - Specify a **date range** for which you need rainfall and temperature data.
        - The system extracts grid points falling within your boundary and converts
          them to SWAT input tables.
    ...
    # ================= NC → CSV + METADATA MODE =================
        ...
        st.header("Upload NetCDF → get CSV and human-readable Metadata")
        st.caption("Extract selected variables from NetCDF files as tabular CSVs with descriptive TXT metadata.")
        ...
        if uploaded:
            ...
            for i, uf in enumerate(uploaded, 1):
                try:
                    ...
                    var_list = list(ds.data_vars.keys())
                    var_sel = st.selectbox(f"Select variable for {uf.name}", var_list, key=f"var_{i}")
                    df_prev = ds[var_sel].to_dataframe(name=var_sel).reset_index().head(8)
                    st.dataframe(df_prev, use_container_width=True)
                    file_configs.append((uf.name, ds, var_sel))
                except Exception as e:
            ...
            if file_configs and st.button("📦 Convert & Download ZIP", type="primary"):
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                        for fname, ds, var in file_configs:
        ...
        st.markdown("""
        This module converts **NetCDF (.nc)** climate datasets into simple CSV tables
        accompanied by easy-to-read metadata summaries.

        **How it works:**
        - Upload one or more NetCDF files.
        - Choose the variable (e.g., rainfall, temperature, soil moisture).
        - Export as `.csv` (data) and `.txt` (metadata) inside a single ZIP.

        **Output Structure:**
        ```
        NetCDF_to_CSV_Metadata/
        ├── filename__variable.csv
        ├── filename__variable__metadata.txt
        └── ...
        ```
        """)

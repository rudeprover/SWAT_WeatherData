...
def find_nc_files():
    ...
    for file in os.listdir(app_dir):
        if file.lower().endswith('.nc'):
            full_path = os.path.join(app_dir, file)
            file_size_mb = os.path.getsize(full_path) / (1024**2)
            nc_files.append({'name': file, 'path': full_path, 'size_mb': file_size_mb})
    return nc_files
...
def build_human_metadata(ds, var_name):
    ...
    for cname, coord in ds.coords.items():
        try:
            ...
            rng = f"n={coord.size}"
        units = getattr(coord, 'attrs', {}).get('units', 'NA')
        lines.append(f"  - {cname} [units={units}]: {rng}")
    lines.append("\n[3] Variables:")
    for vname, dv in ds.data_vars.items():
        dims = ", ".join(list(dv.dims))
        ...
        lines.append(f"  - {vname} (dims: {dims}; units: {units}; long_name: {lname})")
    lines.append(f"\n[4] Selected variable: {var_name}")
    return "\n".join(lines)

# ------------------------------
# Main app using mode switch
# ------------------------------

def main():
    st.set_page_config(page_title="SWAT Weather Data Builder", page_icon="🌦️", layout="wide")
    st.title("🌦️ SWAT Weather Data Builder")
    mode = st.radio("Select Mode:", ["SWAT Builder", "NC → CSV + Metadata"], horizontal=True)
    ...
    if mode == "SWAT Builder":
        ...
        if nc_files:
            st.sidebar.success(f"Found {len(nc_files)} NetCDF files:")
            for f in nc_files:
                st.sidebar.write(f"• {f['name']} ({f['size_mb']:.1f}MB)")
        else:
            st.sidebar.error("No NetCDF files found in app directory!")
            return
        ...
        st.sidebar.header("🗺️ Study Area Boundary")
        app_dir = os.getcwd()
        shapefiles = [f for f in os.listdir(app_dir) if f.lower().endswith(('.geojson', '.shp'))]
        shapefile_source = st.sidebar.radio("Select shapefile source:", ["Upload new file", "Use file from directory"])
        if shapefile_source == "Use file from directory" and shapefiles:
            selected_shapefile = st.sidebar.selectbox("Select shapefile:", shapefiles)
            shapefile_path = os.path.join(app_dir, selected_shapefile)
        else:
            shapefile_upload = st.sidebar.file_uploader("Upload Shapefile (ZIP/GeoJSON)", type=['zip', 'geojson'])
            shapefile_path = None
        ...
        for i, nc in enumerate(nc_files):
            st.write(f"{i+1}. {nc['name']} ({nc['size_mb']:.1f}MB)")

        st.button("🚀 Process All NetCDF Files", type="primary", use_container_width=True)

    else:
        st.header("Upload NetCDF → get CSV and metadata")
        uploaded = st.file_uploader("Upload NetCDF files", type=['nc', 'nc4', 'cdf'], accept_multiple_files=True)
        ...
        if uploaded:
            ...
            for i, uf in enumerate(uploaded, 1):
                try:
                    ...
                    df_prev = ds[var_sel].to_dataframe(name=var_sel).reset_index().head(10)
            ...
            if file_configs and st.button("📦 Convert & Download ZIP", type="primary"):
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with open(zip_path, 'rb') as f:
                        st.download_button("⬇️ Download ZIP", f.read(), file_name="nc_csv_metadata.zip", mime="application/zip")
        else:
            st.info("Upload one or more .nc files to begin.")
